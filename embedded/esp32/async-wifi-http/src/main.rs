...
async fn main(spawner: Spawner) {
    ...
    let (mut controller, interface) = esp_radio::wifi::new(peripherals.WIFI, ControllerConfig::default().with_initial_config(station_config)).unwrap();
    ...
    let tcp_client = TcpClient::new(
        stack,
        mk_static!(TcpClientState<1, 1500, 1500>, TcpClientState::<1, 1500, 1500>::new()),
    );
    ...
    loop {
        ...
        let builder = client
            .request(Method::GET, "http://192.168.31.102:8000/get?hello=Hello+esp-hal")
            .await
            .unwrap();

        let mut builder = builder.headers(&[("Connection", "close")]);
